        ...
        dispatch_async(dispatch_get_global_queue(0, 0), ^{
            ...
            if (mySelf) {
                ...
                dispatch_sync(dispatch_get_main_queue(), ^{
                    ...
                    [mySelf setMovieDecoder:decoder withError:error];
                });
            }
        });
    }
...
//                          forState:UIControlStateNormal];
    ...
    __weak __block typeof(self) mySelf = self;
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, self.timeout * NSEC_PER_SEC), dispatch_get_main_queue(), ^(void) {
        if (!mySelf.playing && mySelf.playCallBack) {
            mySelf.playCallBack(nil, NO);
        }
    });
}
...
- (void) setMoviePosition: (CGFloat) position
{
    ...
    dispatch_after(popTime, dispatch_get_main_queue(), ^(void){
        ...
        [mySelf updatePosition:position playMode:playMode];
    });
}
...
{
    self.alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"Failure", nil)
